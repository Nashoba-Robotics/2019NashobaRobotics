...
public class Drive extends NRSubsystem implements DoublePIDOutput, DoublePIDSource {
	...
	public static final Acceleration MAX_ACCEL_DRIVE = new Acceleration(25, Distance.Unit.FOOT, Time.Unit.SECOND,
			Time.Unit.SECOND);
	public static final Acceleration MAX_ACCEL_DRIVE_H = new Acceleration(10, Distance.Unit.FOOT, Time.Unit.SECOND,
			Time.Unit.SECOND);
	...
	public Distance getHPosition() {
		if (hDrive != null) {
			return new Distance(hDrive.getEncoder().getPosition(), Unit.ENCODER_REV_H); // already integrated for h
																						// drive?
		} else {
			return Distance.ZERO;
		}
		...
		if (leftDrive != null && rightDrive != null && hDrive != null) {
			...
			if (EnabledSubsystems.DRIVE_DUMB_ENABLED) {
				...
				if (Math.abs(hMotorSetpoint.div(MAX_SPEED_DRIVE_H)) < 0.05) {
					...
					hDrive.getPIDController().setReference(
							hMotorSetpoint.get(Distance.Unit.ENCODER_REV_H, Time.Unit.MINUTE), ControlType.kVelocity);
	...
	public void enableMotionProfiler(Distance distX, Distance distY, double maxVelPercent, double maxAccelPercent) {
		...
		if (distX.equals(Distance.ZERO) && !distY.equals(Distance.ZERO)) {
			minVel = MAX_SPEED_DRIVE_H.mul(maxVelPercent).get(Distance.Unit.ENCODER_REV_H,
					Time.Unit.HUNDRED_MILLISECOND);
			...
			minAccel = Math.min(
					(NRMath.hypot(distX, distY).div(distX))
							* MAX_ACCEL_DRIVE.mul(maxAccelPercent).get(Distance.Unit.MAGNETIC_ENCODER_TICK_DRIVE,
									Time.Unit.HUNDRED_MILLISECOND, Time.Unit.HUNDRED_MILLISECOND),
					(NRMath.hypot(distX, distY).div(distY)) * MAX_ACCEL_DRIVE_H.mul(maxAccelPercent).get(
							Distance.Unit.ENCODER_REV_H, Time.Unit.HUNDRED_MILLISECOND, Time.Unit.HUNDRED_MILLISECOND));
		} else {
			minVel = 0;
			minAccel = 0;
	...
	public void smartDashboardInfo() {
		if (leftDrive != null && rightDrive != null) {
			...
			if (EnabledSubsystems.DRIVE_SMARTDASHBOARD_BASIC_ENABLED) {
				...
				SmartDashboard.putNumberArray("Drive H Velocity",
						new double[] { getHVelocity().get(Distance.Unit.FOOT, Time.Unit.SECOND),
								hMotorSetpoint.get(Distance.Unit.FOOT, Time.Unit.SECOND) });
			...
			if (EnabledSubsystems.DRIVE_SMARTDASHBOARD_DEBUG_ENABLED) {
				...
				SmartDashboard.putNumber("Drive Right Encoder Position",
						rightDrive.getSelectedSensorPosition(PID_TYPE));
				...
				DRIVE_RAMP_RATE = new Time(
						...
						Time.Unit.SECOND);
				H_DRIVE_RAMP_RATE = new Time(
						SmartDashboard.getNumber("H Drive Ramp Rate: ", H_DRIVE_RAMP_RATE.get(Time.Unit.SECOND)),
						Time.Unit.SECOND);
